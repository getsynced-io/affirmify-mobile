...
extension View {
    ...
    public func halfASheet<T: View>(isPresented: Binding<Bool> ,isDragable: Bool , @ViewBuilder content: @escaping () -> T) -> some View {
        modifier(HalfASheetPresentationModifier(content: { HalfASheet(isPresented: isPresented, isDragable: isDragable, content: content) }))
    }
}
